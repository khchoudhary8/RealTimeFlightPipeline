    ...
    try:
        ...
        if not daily_df.empty:
            for _, row in daily_df.iterrows():
                print(f"📅 {row['PARTITION_DATE']}: {row['TOTAL_FLIGHTS']:,} flights | "
        ...
        if not country_df.empty:
            for _, row in country_df.iterrows():
                print(f"🇮🇳 {row['ORIGIN_COUNTRY']}: {row['FLIGHT_COUNT']:,} flights "
                      f"({row['PERCENTAGE']}%) | "
                      f"Avg Alt: {row['AVG_ALTITUDE']:.0f}m")
        else:
            print("No data available")
        ...
        if not airline_df.empty:
            for _, row in airline_df.iterrows():
                print(f"🛫 {row['AIRLINE_CODE']} ({row['ORIGIN_COUNTRY']}): "
        ...
        if not stats_df.empty:
            row = stats_df.iloc[0]
            print(f"🔼 Altitude: {row['MIN_ALTITUDE']:.0f}m - {row['MAX_ALTITUDE']:.0f}m "
                  f"(Avg: {row['AVG_ALTITUDE']:.0f}m)")
            print(f"🏃 Velocity: {row['MIN_VELOCITY']:.0f} - {row['MAX_VELOCITY']:.0f} m/s "
        ...
        if not hourly_df.empty:
            for _, row in hourly_df.iterrows():
                ...
                print(f"🕐 {hour:02d}:00 - {hour+1:02d}:00: {row['FLIGHT_COUNT']:,} flights "
                      f"({row['UNIQUE_AIRCRAFT']:,} aircraft)")
        else:
            print("No data available")
