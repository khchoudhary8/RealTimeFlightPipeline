...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("flight_pipeline.log", encoding="utf-8"), UnicodeStreamHandler()],
)
...
╚══════════════════════════════════════════════════════════════╝
    ...
    def _start_component(self, name: str, command: List[str]):
        ...
        try:
            ...
            log_file = f"{name}_component.log"
            ...
            logger.info(f"✅ {name} started (PID: {process.pid}) - Log: {log_file}")
            ...
            while self.running:
                try:
                    ...
                    result = subprocess.run(
                        [python_exec, "Faust/bronze_to_silver.py"], capture_output=True, text=True, timeout=300
                    )
                    if result.returncode == 0:
                    ...
                    result = subprocess.run(
                        [python_exec, "Faust/silver_to_gold.py"], capture_output=True, text=True, timeout=600
                    )
                    if result.returncode == 0:
    ...
    def start_dashboard(self):
        ...
        def dashboard_server():
            ...
            while self.running:
                try:
                    result = subprocess.run(
                        [python_exec, "Faust/analytics_dashboard.py"], capture_output=True, text=True, timeout=60
                    )
                    if result.returncode == 0:
                        print(result.stdout)
                    else:
                ...
                try:
                    for name, process in self.processes.items():
                        if process.poll() is not None:
                            ...
                            # Show last few lines of the component log
                            log_file = f"{name}_component.log"
                            try:
                                if os.path.exists(log_file):
                                    with open(log_file, "r") as f:
                                        lines = f.readlines()
                                        if lines:
                                            logger.error(f"Last 3 lines from {name} log:")
                                            for line in lines[-3:]:
                                                logger.error(f"  {line.strip()}")
                            except Exception as log_error:
                                logger.error(f"Could not read {name} log: {log_error}")

                            # Restart the component
                            try:
                                if name == "producer":
                                    self._start_component(name, "OpenSky/flight-producer.py")
                                elif name == "faust":
                                ...
                                logger.error(f"Failed to restart {name}: {restart_error}")
    ...
    def run_interactive_mode(self):
        ...
        print("  component - Show component logs")
    ...
    def _show_component_logs(self):
        """Show component logs"""
        print("\n📋 Component Logs:")
        print("-" * 50)

        for name in ["producer", "faust"]:
            log_file = f"{name}_component.log"
            if os.path.exists(log_file):
                print(f"\n🔍 {name.upper()} LOGS:")
                try:
                ...
                print(f"\n❌ {name.upper()}: No log file found ({log_file})")
